...
/**
 * Submit CFT session to platform pipeline when linked, else /api/cft-submit.
 * Always safe to call after local archive — marks archive submit status.
 */
export async function submitCftSession(input: {
  ...
  try {
    const row = buildCftSubmissionRow({
      session: input.session,
      trials: input.trials,
      demographics: input.demographics ?? null,
    });

    const res = await fetch(`${base.replace(/\/$/, '')}/api/cft-submit`, {
      method: 'POST',
