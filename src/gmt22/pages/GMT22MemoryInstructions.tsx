...
export default function GMT22MemoryInstructions() {
  const { setPhase, demoMode } = useGMT22State();

  return (
    <div className="page">
      <h1 className="game-title">Memory rounds{demoMode ? ' (demo)' : ''}</h1>
      {demoMode && (
        <p className="hl-demo-badge" role="status">
          Short demo: two blocks, spans 2–3.
        </p>
      )}
      ...
      {demoMode ? (
        <p>
          First block: only X and O. Second block: X and O appear with + — place only X and O
          (ignore the +).
        </p>
      ) : (
        <>
          ...
          >
            <li>
              <strong style={{ color: '#1f77b4' }}>Blue: ignore the +.</strong> Only place X and O.
            </li>
            <li>
              <strong style={{ color: '#ff7f0e' }}>Orange: remember the +.</strong> Place X, O, and +.
            </li>
          </ul>
