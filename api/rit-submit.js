/**
 * Vercel serverless: POST /api/rit-submit
 * Inserts one row into Supabase rit_submissions.
 * Accepts { session, trials, demographics } or a pre-built row with session_id.
 * Field names align with src/rit/lib/submissionPayload.ts / sessionToFlatRow.
 */
...
function buildRow(body) {
  ...
  return {
    ...
    provisional_ics: numOrNull(
      session.provisionalScore?.inhibitoryControlScore ?? session.provisional_ics
    ),
    commission_error_rate: numOrNull(components.commissionErrorRate ?? session.commission_error_rate),
    ...
    post_error_slowing_ms: numOrNull(
      components.postErrorSlowingMs ?? session.post_error_slowing_ms
    ),
    high_interference_accuracy: numOrNull(
      components.highInterferenceAccuracy ?? session.high_interference_accuracy
    ),
    high_interference_mean_rt_ms: numOrNull(
      components.highInterferenceMeanRtMs ?? session.high_interference_mean_rt_ms
    ),
    speed_accuracy_tradeoff: numOrNull(
      components.speedAccuracyTradeoff ?? session.speed_accuracy_tradeoff
    ),
...
export default async function handler(req, res) {
  ...
  const checkRes = await fetch(
    `${supabaseUrl}/rest/v1/rit_submissions?session_id=eq.${encodeURIComponent(sessionId)}&select=session_id`,
  ...
  const response = await fetch(`${supabaseUrl}/rest/v1/rit_submissions`, {
    method: 'POST',
    headers: {
      apikey: supabaseKey,
  ...
  if (!response.ok) {
    ...
    console.error('Supabase rit insert failed', response.status, text);
    let detail = '';
    try {
